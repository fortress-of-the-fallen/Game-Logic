...
namespace Game.Application.Interface
{
    public interface IRestfulService
    {
        ...
        Task<(TResult, HttpStatusCode)> Get<TResult>(string url, Dictionary<string, string> headers = null, int timeout = 30, CancellationToken token = default, bool force = false) where TResult : class;

        Task<(TResult, HttpStatusCode)> Post<TResult>(string url, Dictionary<string, object> formFields = null, Dictionary<string, string> headers = null, int timeout = 30, CancellationToken token = default, bool force = false) where TResult : class;
